...
def _fallback_rag(observation_text, observation):
    try:
            ...
            if similarities[idx] > 0.0:  # Mindest-Threshold
                ...
                if resp and "extended_insight" in resp:
                    for insight in resp["extended_insight"]:
                        ...
                        # ← Nutze das Datum DIESES Insights!
                        ...
                        if key and value:
                            ...
                            knowledge_entries.append({
                                'text': formatted_text,
                                'date': insight_date,  # ← Jedes Insight hat sein eigenes Datum
                                'score': similarities[idx],
                                'session_count': session_count
                            })
                            print(
                                f"✅ Fallback Added (Score: {similarities[idx]:.3f}, Type: {inf_type}, Date: {insight_date}, Sessions: {session_count}): {formatted_text[:70]}...")
        ...
        print(f"\n✅ RAG-prompt generated successfully ({len(knowledge_entries[:10])} units)")
        print(enhanced_prompt)
        return enhanced_prompt

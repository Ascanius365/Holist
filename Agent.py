...
class MinecraftAction(BaseModel):
    ...
    item: Optional[str] = Field(description="The item (e.g. 'berries') or none.")
...
def agent_worker_process(req_q, res_q, log_path, bot_name):
    ...
    system_prompt = (
        "You are a scout in Minecraft trying to survive. "
        f"Your username is '{bot_name}' "
        "Each of your answers must contain a JSON schema. "
    ...
    while True:
        try:
            ...
            try:
                ...
                memory.add_message(
                    input_text=final_feedback if final_feedback else "No tool feedback",
